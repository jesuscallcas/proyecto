			...
			<div class="box box-solid">
				...
				<div class="box-body no-padding">
					<ul class="nav nav-pills nav-stacked">
                    	<li class="active"><a href="<?=site_url('mensajeria/bandeja'); ?>"><i class="fa fa-inbox"></i> Bandeja de entrada <? if ($nummp>0){?><span class="label label-primary pull-right"><?=$nummp?></span><? }?></a></li>
                    	<li><a href="<?=site_url('mensajeria/enviados'); ?>"><i class="fa fa-envelope-o"></i> Enviados</a></li>                    
                  	...
                  	<table class="table table-bordered table-striped table-hover" id="usuarios"> 
                        ...
                        <? foreach($listmensajes as $mensaje):
							if($mensaje['papelera']=='no'){							   
							  if($mensaje['leido']=='no'){	?> 
          						<tr style="font-weight:bold;" data-toggle="modal" data-target="#opc-mensaje" id="<?=$mensaje['idmp']; ?>" onClick="opcMP(<?=$mensaje['idmp']; ?>)">                        
                          			<td class="mailbox-subject"><i class="fa fa-envelope-o"></i> <?=$mensaje['asunto'];?></td>
                          			<td class="mailbox-name"><?=$mensaje['remitente'];?></td>   
                        ...
                        <? 	  }else{?>
                        		<tr data-toggle="modal" data-target="#opc-mensaje" id="<?=$mensaje['idmp']; ?>" onClick="opcMP(<?=$mensaje['idmp']; ?>)">                        
                          			<td class="mailbox-subject"><i class="fa fa-envelope-o"></i> <?=$mensaje['asunto'];?></td>
                          			<td class="mailbox-name"><?=$mensaje['remitente'];?></td>   
                          			<td class="mailbox-date" data-toggle="tooltip" data-placement="bottom" title="<?=$mensaje['fecha_envio'];?>"><?php echo fechainteligente($mensaje['fecha_envio']);?></td>
          						</tr>
			...
			<div class="modal-header" style="background-color:#3c8dbc; color:#FFF" id="header-ventanaleer"></div>
      			<div class="modal-body">         		
        			...
        			<div class="modal-footer">
        				...
        				<button type="button" class="btn btn-primary" data-dismiss="modal" id="brespMP" name="brespMP"><i class="fa fa-envelope-o"></i> Responder</button>        				
        			</div>	
      		</div>      		
   		</div>
 	</div>
</div>
<div class="modal fade" id="respMP" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header" style="background-color:#3c8dbc; color:#FFF" id="header-ventanaResp"></div>
      			<div class="modal-body">         		
            		<form action="<?=site_url('mensajeria/NewMensaje'); ?>" method="post" accept-charset="utf-8" id="newmensaje" name="newmensaje">
                        <div class="form-group" id="divrespasunto">
							<label class="control-label">Asunto</label>
							<input type="text" class="form-control" name="Respasunto" id="Respasunto" pattern="[0-9A-Za-z ñáéíóú/\s/-]{10,100}" value="" required oninput="check_respasunto(this);"/>	
                        </div>
                        <div class="form-group">
                    		<label class="control-label">Destinatario</label>
                      		<input type="text" class="form-control" name="RespDest1" id="RespDest1" value="" disabled/> 
                            <input type="hidden" name="RespDest2" id="RespDest2" value="">                 		
                  		</div>
                    	<div class="form-group" id="divrespmensaje">
                      		<label for="nombre">Mensaje</label>
                        	<textarea class="form-control" rows="3" id="Respmensaje" name="Respmensaje" value="" required oninput="check_respmensaje(this)"></textarea>
                    	</div>
                        <div class="modal-footer">
                        	<button type="button" class="btn btn-danger btn-flat" data-dismiss="modal"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span> Cancelar</button>
                            <button type="submit" class="btn btn-primary"><i class="fa fa-envelope-o"></i> Enviar</button>
                            <input type="hidden" name="Respremitente" id="Respremitente" value="<?=$this->session->userdata('usuario');?>">
                        </div>
					</form>
      			</div>      		
    	   </div>
...
<div class="modal fade bs-example-modal-sm" id="papelera" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			<div class="modal-header" style="background-color:red; color:#FFF">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title" id="myModalLabel">Enviar mensaje a la Papelera</h4>
      		</div>
      		<div class="modal-body" id="body-elim"></div>
            <form action="<?=site_url('mensajeria/papeleraMPAjax'); ?>" method="post" accept-charset="utf-8"> 
                <input type="hidden" name="di" id="di" value="">
            <div class="modal-footer">
        		<button type="button" class="btn btn-danger btn-flat" data-dismiss="modal"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span> Cancelar</button> 
                <button type="submit"  class="btn btn-success btn-flat" id="confElimMP"><spam class='glyphicon glyphicon-ok' aria-hidden="true"></spam> Papelera</button>
            </div> 
          </form>     		
    	</div>
  	</div>
</div>
<div class="modal fade bs-example-modal-sm" id="opc-mensaje" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
            ...
            <div class="modal-footer" id="footer-opc">
            	<button type="button" class="btn btn-success btn-flat" data-dismiss="modal"  data-toggle="modal" data-target="#leer" id="leer-opc"><i class="fa fa-eye"></i> Leer</button>
                <button type="button"  class="btn btn-danger btn-flat" data-dismiss="modal" data-toggle="modal" data-target="#papelera" id="papelera-opc"><i class="fa fa-trash-o"></i> Papelera</button>
            </div> 
    	</div>
  	</div>
</div>
...
function opcMP(di){	
	var di = di;	
	$('#header-opc').html('<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>	</button><h4 class="modal-title" id="myModalLabel"> Acciones para el Mensaje</h4>');
	$('#body-opc').html('<h4 style="color:#060;" align="center">¿Qué desea hacer con este mensaje?</h4>');	
	
	$( "#leer-opc" ).click(function() {				
		...
		$.ajax({	
			...
			success: function(resp){
				if(resp.idmp==di){
					...
					$("#Respasunto").val(resp.asunto);
					$("#RespDest1").val(resp.remitente);  
					$("#RespDest2").val(resp.remitente);  							
				}else{
	...
	$( "#papelera-opc" ).click(function() {
		$('#body-elim').html('<h4 style="color:red;" align="center">¿Realmente desea enviar este mensaje a la papelera?</h4>');	
		$('input[name="di"]').val(di);
	});	
	$( "#brespMP" ).click(function() {
		$('#header-ventanaResp').html('<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>				<h4 class="modal-title" id="myModalLabel"> Panel de respuesta de Mensajes</h4>');	
	});	
	...
	$('#leer').on('hidden.bs.modal', function (e) {
		if (hasBeenClicked == 0) {
    		window.location.href = "http://localhost/icb-academico/index.php/mensajeria/bandeja"; 
		}
	});
};
</script>